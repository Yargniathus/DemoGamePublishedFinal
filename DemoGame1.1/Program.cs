...
namespace DemoGamev1._1
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int playerArmorAC = 0;
            int playerAB = 10;
            int postCombatHealing = rnd.Next(1,8);
            int playerLvl = 1;
            int playerBaseDamage = 2;
            int playerCon = 1;
            int playerMaxHP = 10 + (playerCon + 2) * playerLvl;
            int playerHP = playerMaxHP;
            string playerWpnUsed = "Spoon";
            int playerWpnAB = 1;
            int playerWpnMinBD = 1;
            int playerWpnMaxBD = 2;
        ...
        Precombat:
            ...
            //Console.WriteLine("WeaponBD: " + rnd.Next(wpnMinBD, wpnMaxBD))f; 
            ...
            while (playerHP > 0)
            {
                ...
                int weaponBD = rnd.Next(wpnMinBD, wpnMaxBD);
                int playerWeaponBD = rnd.Next(playerWpnMinBD, playerWpnMaxBD);
                Console.WriteLine(
                ...
                switch (option[0].ToString().ToLower())
                {
                    case "a":
                        ...
                        {
                            Console.WriteLine("You roll " + playerAttackRoll + " + " + playerAB + " against " + crntVillain + "'s armorclass of " + (villainBaseAC + villainArmorAC) + ". You hit " + crntVillain + " with your " + playerWpnUsed + " for " + (playerBaseDamage + playerWeaponBD) + " damage.");
                            crntVillainHp = crntVillainHp - (playerBaseDamage+playerWeaponBD);
                    ...
                    case "h":
                        ...
                        int aux = 1 + (int)(playerHP * 0.2);
                        Console.WriteLine(playerName + " treats wounds for "
                            +aux + " HP." );
                       
                        if ((playerHP + aux) > playerMaxHP)
                        {
                ...
                {
                    Console.WriteLine(crntVillain + " rolls " + villainAttackRoll + " + " + villainAB + " against " + playerName + "'s armorclass of " + (playerBaseAC + playerArmorAC) + ". \n" + crntVillain + " attacks you with " +wpnUsed+ " for " + (villainBD + weaponBD)+ " damage.");
                    playerHP = playerHP - villainBD - weaponBD;
            ...
            Console.WriteLine("The enemy has dropped his " + wpnUsed + ". Do you want to replace your " + playerWpnUsed + " with " + wpnUsed + " (Y/N)?");
            string key2 = Console.ReadLine();
            if (key2.StartsWith("y") | key2.StartsWith("Y"))
            {
                playerWpnUsed = wpnUsed;
                playerWpnAB = wpnAB;
                playerWpnMinBD = wpnMinBD;
                playerWpnMaxBD = wpnMaxBD;
            }
                
                playerHP = playerHP + postCombatHealing;
            Console.WriteLine("You gain " + postCombatHealing + " HP.\n");
            postCombatHealing = rnd.Next(1, 8);
            //nextstage
            if (stage < 10)
            {
